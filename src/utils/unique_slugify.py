...
def unique_slugify(cls, string_to_slugify, restricted_slugs=[]):
    ...
    if cls.objects.filter(slug=slug_value).exists():
        slug_value = unique_slugify(cls,  slug_value + get_random_string(length=4))
    return slug_value